...
const getOptions = async function(context: BuilderContext, browserTarget, overrides: any) {
...
function _runProtractor(
  root: Path,
  options: Partial<SpectronBuilderOptions>,
  context: BuilderContext
): Observable<BuilderOutput> {
  return new Observable<BuilderOutput>(observer => {
    const buildEvent: BuilderOutput = { success: true };
    const executablePath = getSystemPath(
      resolve(normalize(context.workspaceRoot), normalize(options.electronExecutablePath))
    );
    const app = _createSpectronApplication(executablePath);

    app
      .start()
      .then(application => application.client.sessions())
      .then(sessions => {
      ...
      .then(code => {
        return app.client.url('about:blank').then(() =>
          app.stop().then(() => {
      ...
      .catch(error => {
        app.client.url('about:blank').then(() =>
          app.stop().then(() => {
            console.error(error);
