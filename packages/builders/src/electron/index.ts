...
export function runElectronBuilder(
  builderConfig: ElectronBuilderOptions,
  context: BuilderContext
): Observable<BuilderOutput> {
  return of(null).pipe(
    concatMap(() =>
      executeBrowserBuilder(builderConfig, context, {
...
function packElectronApplication(
  builderConfig: ElectronBuilderOptions,
  context: BuilderContext
): Observable<BuilderOutput> {
  ...
  context.logger.info(`Running electron-builder with projectDir: ${args.projectDir} and platforms: ${args.platforms}`);
...
function installElectronApplicationDependencies(
  builderConfig: ElectronBuilderOptions,
  context: BuilderContext
): Observable<BuilderOutput> {
  return runModuleAsObservableFork(
